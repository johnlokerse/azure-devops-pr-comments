...
export class RepositoryDetector {
	...
	private async getRemoteUrl(): Promise<string | undefined> {
		...
		const origin = remotes.find(
			(r) =>
				r.fetchUrl?.includes("azure.com") ||
				r.fetchUrl?.includes("visualstudio.com"),
		);
