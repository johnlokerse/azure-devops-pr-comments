...
export class AzureDevOpsClient {
	...
	async getActivePullRequest(branch: string): Promise<PullRequest | undefined> {
		...
		const data = await this.get<{ value: RawPullRequest[]; count: number }>(
			url,
		);
	...
	async getPullRequestThreads(prId: number): Promise<PullRequestThread[]> {
		...
		return data.value.filter((t) => !t.isDeleted).map((t) => this.mapThread(t));
	}
	...
	async replyToThread(
		...
		const comment = await this.post<RawComment>(url, {
			content: text,
			commentType: 1,
		});
			...
			threadContext: t.threadContext
				? {
						...
						rightFileStart: t.threadContext.rightFileStart
							? {
									line: t.threadContext.rightFileStart.line ?? 1,
									offset: t.threadContext.rightFileStart.offset ?? 1,
								}
							: undefined,
						rightFileEnd: t.threadContext.rightFileEnd
							? {
									line: t.threadContext.rightFileEnd.line ?? 1,
									offset: t.threadContext.rightFileEnd.offset ?? 1,
								}
							: undefined,
						leftFileStart: t.threadContext.leftFileStart
							? {
									line: t.threadContext.leftFileStart.line ?? 1,
									offset: t.threadContext.leftFileStart.offset ?? 1,
								}
							: undefined,
					}
				: undefined,
